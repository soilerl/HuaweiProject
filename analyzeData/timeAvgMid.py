...
class TimeAvgMid:
    ...
    def init_lists(self):
        ...
        for project in self.projects:
            ...
            self.opened_time_avg_ver = []
            self.note_time_avg_ver = []
            self.ended_time_avg_ver = []
    ...
    def rate_calculate(self):
        ...
        for index, project in enumerate(self.projects):
            ...
            if ope_tot == 0:
                self.opened_time_avg_ver[index].append(None)
            else:
                self.opened_time_avg_ver[index].append(ope / ope_tot)

            if note_tot == 0:
                self.note_time_avg_ver[index].append(None)
            else:
                self.note_time_avg_ver[index].append(note / note_tot)

            if end_tot == 0:
                self.ended_time_avg_ver[index].append(None)
            else:
    ...
    def get_df_ope(self):
        return self.opened_time_avg_ver

    def get_df_note(self):
        return self.note_time_avg_ver

    def get_df_end(self):
        return self.ended_time_avg_ver
